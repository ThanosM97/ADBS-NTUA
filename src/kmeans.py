...
# we used Michael Dunn's implementation
# (https://stackoverflow.com/a/4913653/11136838)
...
# Updating the coordinates of the centroids based on
# the points assigned to its cluster (mean value)
...
if __name__ == "__main__":
    ...
    data = data.filter(
        lambda atuple: (
            float(atuple[0]) > -130 and
            float(atuple[0]) < -60 and
            float(atuple[1]) > 20 and
            float(atuple[1]) < 50)
        )
